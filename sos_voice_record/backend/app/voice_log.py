                ...
                if not attachment_info['ok']:
                    ...
                    while len(attachment_info['data']) != len(sosinfo) and i < len(attachment_info['data']):
                        ...
                        cursor.execute("""
                            ...
                            FROM sos_sosinfo ss 
                            WHERE status_id = 5 
                            AND JSON_UNQUOTE(JSON_EXTRACT(call_data, '$.called_data.number')) = '100'
                            AND lastupdate >= %s 
                    ...
                    if len(attachment_info['data']) == len(sosinfo) : 
                        for index, path_data in enumerate(attachment_info['data']):
                            if index <= len(sosinfo):
                                id = sosinfo[index]['id']
                                call_durations = int(sosinfo[index]['call_durations'])
                                call_data = sosinfo[index]['call_data']
                                lastupdate = sosinfo[index]['lastupdate'].replace(tzinfo=tz('UTC'))
                                timestamp = path_data['timestamp'].replace(tzinfo=tz('UTC'))
                                time_diff = (lastupdate - timestamp).total_seconds() - call_durations
                        ...
                        for _, path  in enumerate(attachment_info['data']) :
                            ...
                            for _, source in enumerate(sosinfo):
                                ...
                                lastupdate = source['lastupdate'].replace(tzinfo=tz('UTC'))
                                timestamp = path['timestamp'].replace(tzinfo=tz('UTC'))
                                time_diff = (lastupdate - timestamp).total_seconds() - call_durations
                    ...
                    if len(sos_ids) and len(sos_ids) == len(call_timestamps) : #ในกรณีที่ ข้อมูลที่จะนำไป Map เข้ากับ Path ถูกต้อง
                        check_id = attachment_handler.check_id(sos_ids=sos_ids)
                        if check_id['ok'] :
                            download_attachments = attachment_handler.download_attachments(sos_ids=sos_ids,call_timestamps=call_timestamps)
                            payload = download_attachments
                        else: #ในกรณีที่ sos_id ซ้ำ ต้องลบไฟล์เพื่อ Clear จำนวน Mail ป้องกันการทำงานต่อไปผิดพลาด sos_id =0 จะย้ายไปถังขยะ
                            list_sos_ids = [0] * len(call_timestamps)
                            download_attachments = attachment_handler.download_attachments(sos_ids=list_sos_ids,call_timestamps=call_timestamps)
                            payload = download_attachments
                cursor.close()
